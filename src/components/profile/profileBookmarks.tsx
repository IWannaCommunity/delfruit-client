...
export default function ProfileBookmarks({ user }: UserInfoProps): JSX.Element {
	...
	const fetchBookmarks = useCallback(
		async (
			...
			const res = await API.lists().getBookmarkedGames(
	...
	useEffect(() => {
		...
		const fetchAndSet = async () => {
			const firstPage = await fetchBookmarks(0, sortConfig);
			if (!isCancelled) {
	...
	const loadMore = async () => {
		...
		const moreRatings = await fetchBookmarks(nextPage, sortConfig);
