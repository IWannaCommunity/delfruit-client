...
export default function ProfileFavorites({ user }: UserInfoProps): JSX.Element {
	...
	const fetchFavorites = useCallback(
		async (
			...
			const res = await API.lists().getFavoritesListGames(
				user.id, // id
			);
	...
	useEffect(() => {
		...
		const fetchAndSet = async () => {
			const firstPage = await fetchFavorites(0, sortConfig);
			if (!isCancelled) {
	...
	const loadMore = async () => {
		...
		const moreRatings = await fetchFavorites(nextPage, sortConfig);
